        ...
        <td className="px-3 py-3">
          {editing === 'fressnapf_status' ? (
            <select
              ...
              onBlur={() => saveEdit('fressnapf_status')}
            ...
            <StatusBadge
              value={ticket.fressnapf_status}
              onClick={(e) => startEdit('fressnapf_status', ticket.fressnapf_status, e)}
              editable
            />
          )}
        </td>
        <td className="px-3 py-3">
          {editing === 'amplitude_status' ? (
            <select
              value={editValue}
              onChange={(e) => setEditValue(e.target.value)}
              onBlur={() => saveEdit('amplitude_status')}
              onClick={(e) => e.stopPropagation()}
              className="text-xs border rounded px-1 py-0.5"
              autoFocus
            >
            ...
            <StatusBadge
              value={ticket.amplitude_status}
              onClick={(e) => startEdit('amplitude_status', ticket.amplitude_status, e)}
              editable
            />
          )}
        </td>
      ...
      {expanded && (
        <tr className={rowColor}>
          <td colSpan="8" className="px-6 py-4">
            <ExpandedDetails ticket={ticket} editing={editing} editValue={editValue}
              setEditValue={setEditValue} startEdit={startEdit} saveEdit={saveEdit}
              handleKeyDown={handleKeyDown} onUpdate={onUpdate} />
          </td>
        </tr>
      )}
    </>
...
function ExpandedDetails({ ticket, editing, editValue, setEditValue, startEdit, saveEdit, handleKeyDown, onUpdate }) {
  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm">
      <div className="space-y-2">
        ...
        <DetailRow label="PM Owner" value={ticket.pm_owner} />
      ...
      <div className="space-y-2">
        ...
        <div>
          <div className="flex items-center justify-between">
            ...
            {!noteEdit && (
              <button
                onClick={(e) => { e.stopPropagation(); setNoteEdit(true) }}
          ...
          {noteEdit ? (
            <div className="mt-1" onClick={e => e.stopPropagation()}>
              <textarea
                ...
                className="w-full border rounded p-2 text-xs resize-y min-h-[60px]"
                autoFocus
              />
              <div className="flex gap-2 mt-1">
                <button onClick={saveNote} className="text-xs bg-blue-600 text-white px-2 py-1 rounded">
                  Save
                </button>
